...
class ObsSkymap(Toplevel):
    ...
    def snapshot_airmass_up(self):
        """Showing a snapshot af the airmass at a given time using 3 colors."""
...
############################################################################################        
        ...
        if len(snap_1)!=0:
            ...
            #aladin.send_file('snap_airmass_'+'i')
            aladin.rename(str(time_update))
            #aladin.mv(str(i)+'=<airmass<'+str(j), '"'+'airmass@'+str(time_update)+'"' )
#####################################################################################################
    ...
    if tkMessageBox.askokcancel("Quit", "Do you want to quit?"):
        try:
            temp_files=["GWsky_entries", "GWsky_query_items", "GWsky_fov.vot",
                        "GWsky_config", "GWsky_coords", "obs_airmass_", "uncompress.fits"]
